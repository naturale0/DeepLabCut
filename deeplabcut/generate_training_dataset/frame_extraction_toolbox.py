...
# Add Buttons to the WidgetPanel and bind them to their respective functions.
        ...
        self.slider = wx.Slider(self.widget_panel, id=wx.ID_ANY,
                                value = 0, minValue=0, maxValue=1,size=(200, -1),
...
# Variables initialization
    ...
    def CheckCropping(self):
        ...
        self.crop_coords = list(map(int, self.cfg['video_sets'][videosource]['crop'].split(',')[:4]))
        self.x1, self.x2, self.y1, self.y2 = self.crop_coords

        if self.cropping == True:
# Select ROI of interest by drawing a rectangle
            self.cid = RectangleSelector(self.axes, self.line_select_callback, drawtype='box',
...
# Update the config.yaml file
        self.cfg['video_sets'][self.video_source] = {'crop': ', '.join(map(str, self.crop_coords))}
    ...
    def browseDir(self, event):
        ...
        self.statusbar.SetStatusText(f'Working on video: {os.path.split(str(selectedvideo))[-1]}')
...
# Checks if the video is corrupt.
    ...
    def chooseFrame(self):
        ...
        if output_path.exists() :
            ...
            print(f"{output_path} path not found. Please make sure that the video was added to the config file using the function 'deeplabcut.add_new_videos'.")
